...
import { emitUnreadChanged, CLOUSIGHT_NOTIFICATIONS_REFRESH } from '@/constants/clousightEvents';
...
onMounted(async () => {
  window.addEventListener(CLOUSIGHT_NOTIFICATIONS_REFRESH, onExternalRefresh);
  llmReady.value = await isLlmConfigured();
  await loadList();
});

onBeforeUnmount(() => {
  window.removeEventListener(CLOUSIGHT_NOTIFICATIONS_REFRESH, onExternalRefresh);
});
</script>
